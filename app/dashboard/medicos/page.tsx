  ...
  return (
    <div className="space-y-6 animate-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Médicos y Usuarios</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Gestión de accesos y perfiles profesionales de la clínica.</p>
        </div>
        ...
        <div className="glass p-4 rounded-2xl flex items-center justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <input 
              ...
              className="w-full bg-white dark:bg-slate-950/50 border rounded-xl py-2 pl-11 pr-4 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500/10 transition-all"
            />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-4">
            {filteredDoctors.map((doctor, i) => (
              <motion.div
                ...
                className="group glass p-6 rounded-[2.5rem] shadow-sm hover:shadow-2xl hover:scale-[1.02] transition-all relative border border-slate-100 dark:border-slate-800/50 overflow-hidden"
              >
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center relative group-hover:scale-110 transition-transform duration-500">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white leading-tight">{doctor.name || 'Usuario Admin'}</h3>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter mt-1">{doctor.specialty || 'Administración'}</p>
                    </div>
                  </div>
                  <button className="p-2 hover:bg-white dark:hover:bg-slate-800 rounded-xl transition-all text-slate-400">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                     <button className="p-2.5 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-blue-600 hover:text-white transition-all"><Edit2 className="w-4 h-4" /></button>
